...
- **toggle.sh** — Enables or disables autowake without touching the schedule. Writes `ENABLED` into `local.env`, then brings the agents into line. Rejects an unrecognised argument instead of falling through to "enable", and with no argument only reports the current state.
